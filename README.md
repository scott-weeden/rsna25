...
After extensive research across academic databases, GitHub repositories, and technical documentation, **the "IRIS" segmentation framework for medical imaging does not exist**. The confusion likely stems from iris biometric segmentation systems used for eye recognition. 

**Actual frameworks for spleen segmentation**: The medical imaging field instead relies on **nnU-Net** (self-configuring U-Net achieving >90% Dice scores) and **MONAI** (PyTorch-based framework with pre-trained spleen models achieving 0.961 Dice score on Medical Segmentation Decathlon). These frameworks implement 3D U-Net architectures with automatic preprocessing pipelines, patch-based training (128³-192³ patches), and combined Dice + Cross-entropy loss functions.
